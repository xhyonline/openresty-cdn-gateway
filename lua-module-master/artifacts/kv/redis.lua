...
local db_index = os.getenv("REDIS_DB_INDEX")
local password = os.getenv("REDIS_PASSWORD")
local keepalive = os.getenv("REDIS_KEEPALIVE")  -- 连接保活时间  ms
local pool_size = os.getenv("REDIS_POOL_SIZE") -- 每个 worker 的连接池个数
local timeout = os.getenv("REDIS_TIMEOUT") -- 执行每个命令的超时时间 ms
...
function _M.new(self, opts)
    ...
    if (type(opts) ~= "table") then
        kong.log.err("user_config must be a table")
    ...
    if not (host and port) then
        kong.log.err("no redis server configured host port  is required.")
